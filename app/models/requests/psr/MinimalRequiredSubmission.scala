...
case class SchemeDesignatory(
                              recordVersion: Option[String],
                            reportableAsset: Boolean
                            )

object MinimalRequiredSubmission {
   implicit val reportDetailsFormat: OFormat[RDetails] = Json.format[RDetails]
   implicit val formatAccountingPeriodDetails: OFormat[AccountingPeriodDetails] =
    Json.format[AccountingPeriodDetails]
  implicit val schemeDesignatoryFormat: OFormat[SchemeDesignatory] = Json.format[SchemeDesignatory]
