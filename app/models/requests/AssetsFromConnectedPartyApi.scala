...
object AssetsFromConnectedPartyApi {
  ...
  implicit val assetsTrxDetailRowEncoder: RowEncoder[TransactionDetail] = RowEncoder.instance { trx =>
    NonEmptyList.of(
      ...
      trx.shareCompanyDetails.map(_.companySharesName).getOrElse(""),
      trx.shareCompanyDetails.flatMap(_.companySharesCRN.map(_.crn)).getOrElse(""),
      trx.shareCompanyDetails.flatMap(_.reasonNoCRN).getOrElse(""),
      trx.shareCompanyDetails.map(_.sharesClass).getOrElse(""),
      trx.shareCompanyDetails.map(_.noOfShares.toString).getOrElse(""),
