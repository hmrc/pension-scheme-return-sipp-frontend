...
class FormBundleRequiredActionImpl @Inject() (implicit val executionContext: ExecutionContext)
  ...
  override protected def refine[A](request: DataRequest[A]): Future[Either[Result, FormBundleRequest[A]]] =
    FormBundleNumber.optFromSession(request.session) match {
      case None =>
        Future.successful(Left(Redirect(routes.JourneyRecoveryController.onPageLoad())))
      case Some(data) =>
        Future.successful(Right(FormBundleRequest(data, request)))
    }

}

trait FormBundleRequiredAction extends ActionRefiner[DataRequest, FormBundleRequest]
