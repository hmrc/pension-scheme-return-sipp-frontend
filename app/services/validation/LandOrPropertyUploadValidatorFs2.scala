...
class LandOrPropertyUploadValidatorFs2 @Inject()(
  uploadValidatorFs2: UploadValidatorFs2,
  ...
  private def validateJourney(
    ...
    (for {
      ...
      case None =>
        CsvRowInvalid(row, NonEmptyList.of(ValidationError(row, InvalidRowFormat, "the row was bad ;)")), data)
      case Some((_, Valid(landConnectedProperty))) => CsvRowValid(row, landConnectedProperty, data)
      case Some((_, Invalid(errs))) => CsvRowInvalid[LandOrConnectedPropertyRequest.TransactionDetail](row, errs, data)
    }
  }
