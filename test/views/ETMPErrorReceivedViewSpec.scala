...
//    val document: Document = Jsoup.parse(view.url.toString())
    ...
    "ETMPErrorReceivedView" - {
      ...
      "render the first paragraph" in {
        Jsoup.parse(view().body).getElementsByClass("govuk-body").first() must startWith(Messages("serviceUnavailable.paragraph1"))
      }
      "render the second paragraph" in {
        Jsoup.parse(view().body).getElementsByClass("govuk-body").last must startWith(Messages("serviceUnavailable.paragraph1"))
      }
    }
